...
function s.rvlimit(e) return not e:GetHandler():IsLocation(LOCATION_HAND) end
function s.psplimit(e, c, tp, sumtp, sumpos) return not c:IsRace(RACE_DRAGON) and (sumtp & SUMMON_TYPE_PENDULUM) == SUMMON_TYPE_PENDULUM end
...
function s.splimit(e, se, sp, st) return (st & SUMMON_TYPE_PENDULUM) == SUMMON_TYPE_PENDULUM and e:GetHandler():IsLocation(LOCATION_HAND) end
function s.hspfilter(c, e, tp) return c:IsRace(RACE_DRAGON) and c:IsType(TYPE_FUSION + TYPE_SYNCHRO + TYPE_XYZ) end
function s.hspcon(e, c)
    ...
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > -3 and #g1 > 0 and #g2 > 0 and #g3 > 0 and
               aux.SelectUnselectGroup(g1, e, tp, 1, 1, aux.ChkfMMZ(1), 0) and aux.SelectUnselectGroup(g2, e, tp, 1, 1, aux.ChkfMMZ(1), 0) and
               aux.SelectUnselectGroup(g3, e, tp, 1, 1, aux.ChkfMMZ(1), 0)
end
...
function s.thfilter(c) return c:IsType(TYPE_PENDULUM) and c:IsRace(RACE_DRAGON) and c:IsLevelBelow(8) and c:IsAbleToHand() end
...
function s.atkval(e, c) return math.floor(Duel.GetLP(1 - e:GetHandlerPlayer()) / 2) end
