...
function s.e1filter(c, e, tp)
    return c:IsLevelBelow(8) and c:IsSetCard(SET_RED_DRAGON_ARCHFIEND) and c:IsType(TYPE_SYNCHRO) and
               c:IsCanBeSpecialSummoned(e, SUMMON_TYPE_SYNCHRO, tp, false, false) and Duel.GetLocationCountFromEx(tp, tp, e:GetHandler(), c) > 0
end
...
function s.e2filter(c)
    return c:IsSpellTrap() and (c:ListsCode(CARD_RED_DRAGON_ARCHFIEND) or c:ListsArchetype(SET_RED_DRAGON_ARCHFIEND)) and
