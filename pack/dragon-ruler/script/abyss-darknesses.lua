-- Abyss, Dragon Ruler of Darknesses
...
function s.e1filter(c)
    return (c:IsRace(RACE_DRAGON) or c:IsAttribute(ATTRIBUTE_DARK)) and c:IsAbleToRemoveAsCost() and
...
function s.e3filter(c) return c:IsAttribute(ATTRIBUTE_DARK) and c:IsDiscardable() and c:IsAbleToGraveAsCost() end

function s.e3cost(e, tp, eg, ep, ev, re, r, rp, chk)
    local c = e:GetHandler()
    if chk == 0 then return c:IsDiscardable() and c:IsAbleToGraveAsCost() and Duel.IsExistingMatchingCard(s.e3filter, tp, LOCATION_HAND, 0, 1, c) end

    Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_DISCARD)
    local g = Duel.SelectMatchingCard(tp, s.e3filter, tp, LOCATION_HAND, 0, 1, 1, c)
    g:AddCard(c)
...
function s.e4filter(c) return c:IsRace(RACE_DRAGON) and c:IsAttribute(ATTRIBUTE_DARK) and c:IsAbleToHand() end
