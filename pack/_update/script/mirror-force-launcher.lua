...
function s.thcon(e, tp, eg, ep, ev, re, r, rp) return Duel.IsTurnPlayer(tp) and Duel.IsMainPhase() end
function s.thcfilter(c) return c:IsMonster() and c:IsDiscardable() end
...
function s.thfilter(c) return c:IsCode(44095762) and c:IsAbleToHand() end
function s.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
    if chk == 0 then
        return Duel.IsExistingMatchingCard(s.thfilter, tp, LOCATION_DECK | LOCATION_GRAVE, 0, 1, nil) and e:GetHandler():GetFlagEffect(id) == 0
    end
...
function s.setfilter(c) return c:IsCode(44095762) and c:IsSSetable() end
function s.settg(e, tp, eg, ep, ev, re, r, rp, chk)
    local c = e:GetHandler()
    if chk == 0 then
        return Duel.GetLocationCount(tp, LOCATION_SZONE) > 1 and c:IsSSetable() and
                   Duel.IsExistingMatchingCard(s.setfilter, tp, LOCATION_HAND | LOCATION_DECK | LOCATION_GRAVE, 0, 1, nil)
    end
