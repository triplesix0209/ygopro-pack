...
function s.initial_effect(c)
    ...
    -- fusion summon
    Fusion.AddProcMixN(c, false, false, function(c, fc, st, tp) return c:IsLevel(7) and c:IsSetCard(SET_RED_EYES, fc, st, tp) end, 3)
...
function s.e3filter(c) return c:IsFaceup() and c:IsSetCard(SET_RED_EYES) and c:IsMonster() and c:IsAbleToDeck() end
