...
s.listed_names = {16178681, 900005027}
...
function s.e1filter2(c, e, tp, odd)
    return c:IsType(TYPE_XYZ) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and
               Duel.IsExistingMatchingCard(s.e1filter3, tp, LOCATION_EXTRA, 0, 1, odd, e, tp, odd, c)
end

function s.e1filter3(c, e, tp, odd, xyz)
    ...
    ec1:SetCode(EFFECT_XYZ_LEVEL)
    ec1:SetValue(odd:GetLevel())
    ec1:SetReset(RESET_EVENT + RESETS_STANDARD)
    xyz:RegisterEffect(ec1)
    local result = c:IsXyzSummonable(nil, Group.FromCards(odd, xyz), 2, 2)
    ec1:Reset()
    return result
end
...
function s.e1op(e, tp, eg, ep, ev, re, r, rp)
    ...
    local g =
        Duel.GetMatchingGroup(function(sc) return sc:IsType(TYPE_PENDULUM) and sc:IsXyzSummonable(nil, sg, 2, 2) end, tp, LOCATION_EXTRA, 0, nil)
    if #g > 0 then
        Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_SPSUMMON)
        local sc = g:Select(tp, 1, 1, nil):GetFirst()
        Duel.XyzSummon(tp, sc, nil, sg)
...
function s.e1disop(c, tc, lv)
    ...
    if not tc:IsCode(16178681) then
        ...
        ec2:SetCode(EFFECT_RANK_LEVEL_S)
        ec2:SetReset(RESET_EVENT + RESETS_STANDARD)
        tc:RegisterEffect(ec2)
        local ec2b = ec2:Clone()
        ec2b:SetCode(EFFECT_CHANGE_LEVEL)
        ec2b:SetValue(lv)
