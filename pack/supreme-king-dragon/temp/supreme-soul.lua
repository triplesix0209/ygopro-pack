...
function s.e6filter1(c, e, tp, sg)
...
function s.e6rescon(checkfunc)
    return function(sg, e, tp, mg)
        if not sg:CheckDifferentProperty(checkfunc) then return false, true end
        return Duel.IsExistingMatchingCard(s.e6filter1, tp, LOCATION_EXTRA, 0, 1, nil, e, tp, sg)
    end
end

function s.e6cost(e, tp, eg, ep, ev, re, r, rp, chk)
    local c = e:GetHandler()
    local mg = Duel.GetMatchingGroup(s.e6filter2, tp, LOCATION_HAND + LOCATION_MZONE + LOCATION_GRAVE + LOCATION_EXTRA, 0, c)
    local checkfunc = aux.PropertyTableFilter(Card.GetSetCard, 0x10f2, 0x2073, 0x2017, 0x1046)
    if chk == 0 then return aux.SelectUnselectGroup(mg, e, tp, 4, 4, s.e6rescon(checkfunc), 0) end

    local sg = aux.SelectUnselectGroup(mg, e, tp, 4, 4, s.e6rescon(checkfunc), 1, tp, HINTMSG_REMOVE, s.e6rescon(checkfunc))
...
function s.e6op(e, tp, eg, ep, ev, re, r, rp)
    ...
    local tc = Utility.SelectMatchingCard(HINTMSG_SPSUMMON, tp, s.e6filter1, tp, LOCATION_EXTRA, 0, 1, 1, nil, e, tp):GetFirst()
    if tc then
