...
function s.e1op(e, tp, eg, ep, ev, re, r, rp)
    ...
    for tc in aux.Next(g1) do Duel.MoveToField(tc, tp, tp, LOCATION_PZONE, POS_FACEUP, true) end
...
function s.e5filter1(c, e, tp, sg)
    return c:IsCode(13331639) and c:IsCanBeSpecialSummoned(e, 0, tp, true, false) and Duel.GetLocationCountFromEx(tp, tp, sg or nil, c) > 0
end

function s.e5filter2(c)
    if c:IsLocation(LOCATION_EXTRA) and c:IsFacedown() then return false end
    return c:IsSetCard({0x10f2, 0x2073, 0x2017, 0x1046}) and c:IsType(TYPE_MONSTER) and c:IsAbleToRemoveAsCost() and
               (not c:IsLocation(LOCATION_MZONE) or c:IsFaceup()) and (c:IsLocation(LOCATION_HAND) or aux.SpElimFilter(c, true, true))
end

function s.e5rescon(checkfunc)
    return function(sg, e, tp, mg)
        if not sg:CheckDifferentProperty(checkfunc) then return false, true end
        return Duel.IsExistingMatchingCard(s.e5filter1, tp, LOCATION_EXTRA, 0, 1, nil, e, tp, sg)
    end
end

function s.e5cost(e, tp, eg, ep, ev, re, r, rp, chk)
    local c = e:GetHandler()
    local mg = Duel.GetMatchingGroup(s.e5filter2, tp, LOCATION_HAND + LOCATION_MZONE + LOCATION_GRAVE + LOCATION_EXTRA, 0, c)
    local checkfunc = aux.PropertyTableFilter(Card.GetSetCard, 0x10f2, 0x2073, 0x2017, 0x1046)
    if chk == 0 then return aux.SelectUnselectGroup(mg, e, tp, 4, 4, s.e5rescon(checkfunc), 0) end

    local sg = aux.SelectUnselectGroup(mg, e, tp, 4, 4, s.e5rescon(checkfunc), 1, tp, HINTMSG_REMOVE, s.e5rescon(checkfunc))
...
function s.e5op(e, tp, eg, ep, ev, re, r, rp)
    ...
    local tc = Utility.SelectMatchingCard(HINTMSG_SPSUMMON, tp, s.e5filter1, tp, LOCATION_EXTRA, 0, 1, 1, nil, e, tp):GetFirst()
    if tc then
