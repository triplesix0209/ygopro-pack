-- Supreme King Dragon Silverwurm
...
function s.initial_effect(c)
    ...
    pe1:SetValue(function(e, re, rp) return re:IsActiveType(TYPE_SPELL) and rp ~= e:GetHandlerPlayer() end)
...
function s.me1filter(c) return c:IsAttribute(ATTRIBUTE_DARK) and c:IsType(TYPE_PENDULUM) and not c:IsForbidden() end

function s.me1tg(e, tp, eg, ep, ev, re, r, rp, chk) if chk == 0 then return Duel.IsExistingMatchingCard(s.me1filter, tp, LOCATION_DECK, 0, 1, nil) end end
