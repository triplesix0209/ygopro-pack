...
function s.initial_effect(c)
    ...
    e4:SetTarget(function(e, tc) return tc == e:GetHandler() or (tc:IsFaceup() and tc:IsType(TYPE_CONTINUOUS)) end)
    e4:SetValue(function(e, te)
        local tc = te:GetOwner()
        return tc ~= e:GetOwner() and tc:IsSetCard(SET_NEOS)
    end)
    c:RegisterEffect(e4)
end
...
function s.e2op(e, tp, eg, ep, ev, re, r, rp)
    ...
    local loc = LOCATION_HAND + LOCATION_DECK + LOCATION_EXTRA + LOCATION_GRAVE
    local ft = Duel.GetLocationCount(tp, LOCATION_MZONE)
    local tc = Utility.SelectMatchingCard(HINTMSG_SPSUMMON, tp, s.e2filter2, tp, loc, 0, 1, 1, nil, e, tp, ft):GetFirst()
    if tc then
