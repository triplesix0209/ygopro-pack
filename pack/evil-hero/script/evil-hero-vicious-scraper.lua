...
function s.initial_effect(c)
    ...
    -- fusion summon
    Fusion.AddProcMix(c, true, true, aux.FilterBoolFunctionEx(Card.IsSetCard, SET_EVIL_HERO), aux.FilterBoolFunctionEx(Card.IsLevelBelow, 4))
...
function s.e1tg(e, tp, eg, ep, ev, re, r, rp, chk)
    ...
    if chk == 0 then return Duel.IsExistingMatchingCard(s.e1filter, tp, loc, 0, 1, nil, c) and Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 end
...
function s.e1op(e, tp, eg, ep, ev, re, r, rp)
    ...
    local tc = Utility.SelectMatchingCard(HINTMSG_EQUIP, tp, aux.NecroValleyFilter(s.e1filter), tp, LOCATION_HAND + LOCATION_DECK + LOCATION_GRAVE, 0,
        1, 1, nil, c):GetFirst()
